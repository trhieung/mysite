...
void CommandHandler::handler_func2(const std::vector<std::string> &cmds)
{
    ...
    if(cmds[1] != "-u"){
        std::cout << "Missing -u flag!, try again!\n";
        return;
    }
    for (size_t i = 2; i < cmds.size(); ++i) // Start from 1 to skip the command itself
    {
    ...
    curl_handler->num_activate_download = curl_handler->curlsData.size();
    for (size_t i = 0; i < temp_urls.size(); ++i)
    {
        if (curl_handler->check_url_format(temp_urls[i]))
        {
            curl_handler->add_download_task(static_cast<unsigned short>(i + curl_handler->num_activate_download), temp_urls[i], des_folder);
        }
    }

    // begin downloads in multiple threads like multiple tasks;
    curl_handler->start_downloads();
}
